...
from tools import *
import json
from prompts.book import zh_hierarchical_summarization_prompt, en_hierarchical_summarization_prompt
...
class SummaryBot(object):
    ...
    def export_history(self, book_file, suffix=''):
    ...
    def _summarize_paragraphs(self, paragraphs: str, max_tokens):
        ...
        lang2template = {
            LANG_EN: en_hierarchical_summarization_prompt,
            LANG_ZH: zh_hierarchical_summarization_prompt
        }

        tmp: str = choose_language_template(lang2template, paragraphs)
        input_text = tmp.format(paragraph_summaries=paragraphs_text, max_tokens=max_tokens)
